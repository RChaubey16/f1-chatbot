...
async def run_live(since: datetime | None = None) -> IngestionResult:
    chunker = Chunker()
    embedder = OllamaEmbedder()
    loader = PgVectorLoader()
    ...
    extractors = [
        ...
        NewsExtractor(since=since, url_exists_fn=loader.url_exists),
    ]

    progress = tqdm(desc="Live ingestion", unit="doc")
    ...
    try:
        ...
        chunk_count = await loader.get_chunk_count()
        log.info("pipeline.live.done", total_chunks=chunk_count, summary=total.summarise())
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Only ingest live data newer than this ISO date (e.g. 2024-01-01). Live phase only.",
    )
    ...
    if args.phase in ("live", "all"):
        result = asyncio.run(run_live(since=since))
        print(f"\nLive ingestion complete: {result.summarise()}")
